...
class EnhancedConversionResult:
    ...
    test_code: str = ""
    parse_details: Optional[SPLParseResult] = None


# SPL commands that require aggregation (must be scheduled)
AGGREGATION_COMMANDS = {
    'stats', 'eventstats', 'streamstats', 'chart', 'timechart',
    'top', 'rare', 'bin', 'bucket', 'cluster', 'kmeans',
    'transaction', 'concurrency', 'sistats', 'sichart', 'sitop',
    'sitimechart', 'sirare', 'geostats', 'tstats', 'mstats',
    'addtotals', 'autoregress', 'trendline', 'predict', 'x11',
    'contingency', 'correlate', 'diff', 'outlier', 'anomalydetection'
}

# SPL commands that can work in streaming mode
STREAMING_COMMANDS = {
    'search', 'where', 'eval', 'rex', 'spath', 'xpath', 'extract',
    'fields', 'rename', 'replace', 'fillnull', 'convert', 'fieldformat',
    'lookup', 'regex', 'case', 'validate', 'tags', 'typer', 'reltime',
    'iplocation', 'geom', 'iconify', 'highlight', 'rangemap',
    'makemv', 'nomv', 'mvexpand', 'split'
}

# SPL commands for output/display (can be ignored or simplified)
OUTPUT_COMMANDS = {
    'table', 'sort', 'head', 'tail', 'reverse', 'dedup', 'uniq',
    'outputcsv', 'outputtext', 'sendemail', 'collect', 'tscollect',
    'format', 'return', 'addinfo'
}
...
class EnhancedSPLConverter:
    ...
    def _detect_functions(self, expr: str) -> List[str]:
        ...
        spl_functions = [
            'coalesce', 'if', 'case', 'match', 'like', 'cidrmatch',
            'lower', 'upper', 'len', 'substr', 'replace', 'trim', 'ltrim', 'rtrim',
            'split', 'mvcount', 'mvindex', 'mvfilter', 'mvjoin', 'mvsort', 'mvzip',
            'now', 'time', 'strftime', 'strptime', 'relative_time',
            'tonumber', 'tostring', 'typeof',
            'abs', 'ceil', 'floor', 'round', 'sqrt', 'pow', 'log', 'ln', 'exp',
            'min', 'max', 'sum', 'avg', 'count', 'dc', 'values', 'list',
            'isnull', 'isnotnull', 'nullif', 'null', 'true', 'false',
            'md5', 'sha1', 'sha256', 'sha512',
            'urldecode', 'urlencode', 'base64decode', 'base64encode',
            'json_extract', 'json_array', 'json_object', 'json_valid',
            'spath', 'mvexpand', 'mvappend', 'mvdedup', 'mvrange',
            'searchmatch', 'commands', 'typeof', 'validate'
        ]
    ...
    def _parse_sort(self, content: str) -> dict:
        ...
        for field in content.split(','):
            ...
            if not field:
                continue
            if field.startswith('-'):
                result["fields"].append({"field": field[1:].strip(), "order": "desc"})
            elif field.startswith('+'):
                result["fields"].append({"field": field[1:].strip(), "order": "asc"})
            else:
                result["fields"].append({"field": field, "order": "asc"})

        return result

    def _parse_dedup(self, content: str) -> dict:
        """Parse a dedup command."""
        result = {"fields": [], "keepevents": False, "keepempty": False, "consecutive": False, "sortby": None}
    ...
    def _parse_top(self, content: str) -> dict:
        ...
        result = {"limit": 10, "fields": [], "by_fields": [], "countfield": "count", "percentfield": "percent"}
    ...
    def _parse_join(self, content: str) -> dict:
        ...
        if '[' in content:
            ...
            result["fields"] = [f.strip() for f in fields_part.split(',') if f.strip() and '=' not in f]
    ...
    def _parse_transaction(self, content: str) -> dict:
        ...
        result = {"fields": [], "startswith": None, "endswith": None, "maxspan": None, "maxpause": None, "keepevicted": False}

        # Extract startswith
        startswith_match = re.search(r'startswith\s*=\s*("([^"]+)"|\'([^\']+)\'|(\S+))', content, re.IGNORECASE)
        if startswith_match:
            result["startswith"] = startswith_match.group(2) or startswith_match.group(3) or startswith_match.group(4)
            content = content[:startswith_match.start()] + content[startswith_match.end():]

        # Extract endswith
        endswith_match = re.search(r'endswith\s*=\s*("([^"]+)"|\'([^\']+)\'|(\S+))', content, re.IGNORECASE)
        if endswith_match:
            result["endswith"] = endswith_match.group(2) or endswith_match.group(3) or endswith_match.group(4)
    ...
    def _parse_fillnull(self, content: str) -> dict:
        ...
        # Extract value
        value_match = re.search(r'value\s*=\s*("([^"]+)"|\'([^\']+)\'|(\S+))', content, re.IGNORECASE)
        if value_match:
            result["value"] = value_match.group(2) or value_match.group(3) or value_match.group(4) or ""
    ...
    def _parse_replace(self, content: str) -> dict:
        ...
        # Parse replacements
        with_matches = re.findall(r'("([^"]+)"|\'([^\']+)\'|(\S+))\s+WITH\s+("([^"]+)"|\'([^\']+)\'|(\S+))', content, re.IGNORECASE)
    ...
    def _parse_convert(self, content: str) -> dict:
        ...
        # Extract timeformat
        tf_match = re.search(r'timeformat\s*=\s*("([^"]+)"|\'([^\']+)\'|(\S+))', content, re.IGNORECASE)
        if tf_match:
    ...
    def _parse_makemv(self, content: str) -> dict:
        ...
        # Extract delim
        delim_match = re.search(r'delim\s*=\s*("([^"]+)"|\'([^\']+)\'|(\S+))', content, re.IGNORECASE)
        if delim_match:
        ...
        # Extract tokenizer
        tokenizer_match = re.search(r'tokenizer\s*=\s*("([^"]+)"|\'([^\']+)\')', content, re.IGNORECASE)
        if tokenizer_match:
    ...
    def _convert_span_to_sql(self, span: str, field: str = "_time") -> dict:
        ...
        unit_map = {
            ...
            'second': ('second', 'seconds'), 'seconds': ('second', 'seconds'),
            ...
            'd': ('day', 'days'), 'day': ('day', 'days'), 'days': ('day', 'days'),
            'w': ('week', 'weeks'), 'week': ('week', 'weeks'), 'weeks': ('week', 'weeks'),
            'mon': ('month', 'months'), 'month': ('month', 'months'), 'months': ('month', 'months'),
            'y': ('year', 'years'), 'year': ('year', 'years'), 'years': ('year', 'years'),
        }
...
''')
    ...
    def _generate_streaming_rule(
        ...
        if has_aggregation:
            todos.append("WARNING: This SPL has aggregations (stats/count/dc) which require a Scheduled Rule with SQL, or use Panther Lookup Tables for state tracking")
        ...
        for cmd in parsed.commands:
            if cmd.name == 'eval':
                ...
                if extracted:
                    ...
                    eval_code_lines.append(f"        _match = re.search(r'{pattern}', deep_get(event, '{field}', ''))")
                    for ef in extracted:
                        eval_code_lines.append(f"        {ef} = _match.group('{ef}') if _match else None")
                else:
                ...
                if path:
                    ...
                    eval_code_lines.append(f"        {output} = deep_get(event, '{path_str}', None)")

            elif cmd.name == 'lookup':
                lookup_name = cmd.args.get('lookup_name', '')
                todos.append(f"Implement lookup table '{lookup_name}' - use p_enrichment() or Panther Lookup Tables")

            elif cmd.name == 'iplocation':
                field = cmd.args.get('field', 'clientip')
                todos.append(f"Add geolocation for '{field}' - use Panther's p_enrichment() or GeoIP lookup")

            elif cmd.name in ('stats', 'eventstats', 'streamstats', 'timechart', 'chart', 'top', 'rare'):
                # Can't do aggregation in streaming - add detailed TODO
                pass  # Already handled above

            elif cmd.name == 'transaction':
                todos.append("Transaction correlation requires Scheduled Rule or correlation logic outside the rule")

            elif cmd.name not in ('search', 'where', 'fields', 'table', 'rename', 'sort', 'head', 'tail', 'dedup'):
        ...
        code_lines.extend([
            ...
            f"        return f\"{class_name}: {{deep_get(event, '{title_field}', 'unknown')}}\"",
            '',
        ])
    ...
    def _generate_scheduled_rule(
        ...
        for cmd in parsed.commands:
            if cmd.name in ('stats', 'eventstats', 'streamstats'):
                ...
                todos.append(f"Implement transaction logic with window functions or self-join")
        ...
        time_filter = self._convert_time_range(parsed.earliest) if parsed.earliest else "p_occurs_since('1 hour')"
        ...
        for part in sql_parts:
            ...
            elif any(agg in part.upper() for agg in ['COUNT(', 'SUM(', 'AVG(', 'MAX(', 'MIN(', 'ARRAY_AGG(']):
        ...
        if computed_columns:
            ...
            outer_select = ',\n        '.join(aggregation_columns) if aggregation_columns else 'COUNT(*) AS count'
            if group_by_fields:
    ...
    WHERE
        {where_clause}
    {('GROUP BY ' + sql_group) if sql_group else '-- TODO: Add GROUP BY if needed'}
    {('HAVING ' + ' AND '.join(having_conditions)) if having_conditions else '-- TODO: Add HAVING for thresholds'}
    """
        ...
        return EnhancedConversionResult(
            ...
            recommendation_reasons=reasons if reasons else ["Contains aggregation that requires scheduled execution"],
            test_code=self._generate_test_code(class_name, log_type),
            parse_details=parsed,
        )

    def _convert_agg_to_sql(self, agg: dict) -> Optional[str]:
    ...
    def _convert_eval_to_sql(self, field: str, expr: str) -> Optional[str]:
        ...
        upper_coalesce_match = re.match(r'^upper\s*\(\s*coalesce\s*\((.+)\)\s*\)$', expr, re.IGNORECASE)
    ...
    def _convert_condition_to_sql(self, condition: str) -> Optional[str]:
    ...
    def _convert_eval_to_python(self, field: str, expr: str) -> Optional[str]:
    ...
    def _convert_where_to_python(self, where_cond: str) -> Optional[str]:
    ...
    def _get_title_field(self, parsed: SPLParseResult) -> str:
        ...
        common_fields = ['hostname', 'host', 'ComputerName', 'src_ip', 'dest_ip', 'user', 'userName']
    ...
    def _generate_threshold_check(self, where_conditions: List[str], group_by_fields: List[str]) -> List[str]:
