...
class TimeRange:
    ...
    def to_sql(self) -> str:
        unit_map = {"MINUTE": "MINUTE", "HOUR": "HOUR", "DAY": "DAY", "WEEK": "WEEK", "MONTH": "MONTH"}
        return f"timestamp >= NOW() - INTERVAL {self.value} {unit_map.get(self.unit.upper(), 'HOUR')}"

    def to_panther(self) -> str:
        unit_map = {"MINUTE": "minutes", "HOUR": "hours", "DAY": "days", "WEEK": "weeks", "MONTH": "days"}
        multiplier = {"MINUTE": 1, "HOUR": 1, "DAY": 1, "WEEK": 7, "MONTH": 30}
        val = self.value * multiplier.get(self.unit.upper(), 1)
    ...
    def to_cql(self) -> str:
        ...
        multiplier = {"MINUTE": 1, "HOUR": 1, "DAY": 1, "WEEK": 7, "MONTH": 30}
        val = self.value * multiplier.get(self.unit.upper(), 1)
        return f"| filter timestamp.seconds > timestamp.current_timestamp() - {val * 60 if unit_map.get(self.unit.upper()) == 'm' else val * 3600}"
...
class Aggregation:
    ...
    def to_spl(self) -> str:
        func_map = {"COUNT": "count", "SUM": "sum", "AVG": "avg", "MIN": "min", "MAX": "max", "UNIQUECOUNT": "dc"}
    ...
    def to_kql(self) -> str:
        func_map = {"COUNT": "count", "SUM": "sum", "AVG": "avg", "MIN": "min", "MAX": "max", "UNIQUECOUNT": "dcount"}
        func = func_map.get(self.function.upper(), self.function.lower())
        if self.field:
            expr = f"{func}({self.field})"
        else:
    ...
    def to_sql(self) -> str:
        func_map = {"COUNT": "COUNT", "SUM": "SUM", "AVG": "AVG", "MIN": "MIN", "MAX": "MAX", "UNIQUECOUNT": "COUNT(DISTINCT"}
    ...
    def to_cql(self) -> str:
        ...
        func_map = {"COUNT": "count", "SUM": "sum", "AVG": "avg", "MIN": "min", "MAX": "max", "UNIQUECOUNT": "count_distinct"}
        func = func_map.get(self.function.upper(), self.function.lower())
        if self.field:
            expr = f"{func}({self.field})"
        else:
...
class SigmaRule:
    ...
    having: Optional[str] = None  # HAVING clause condition
...
class SigmaToYARAL(FromSigmaConverter):
    ...
    def convert(self, sigma: SigmaRule) -> str:
        ...
        if event_conditions:
            ...
            lines.append(f'    // TODO: Add event matching conditions')
            lines.append(f'    // Original query had no WHERE clause filters')
    ...
    def _convert_streaming_rule(self, sigma: SigmaRule) -> str:
        ...
        for key, value in selection.items():
            ...
            if isinstance(value, list):
                ...
                for v in value:
                    if modifier == 'contains':
                        value_checks.append(f'"{v}".lower() in event.get("{panther_field}", "").lower()')
                    elif modifier == 'endswith':
                        value_checks.append(f'event.get("{panther_field}", "").lower().endswith("{v}".lower())')
                    elif modifier == 'startswith':
                        value_checks.append(f'event.get("{panther_field}", "").lower().startswith("{v}".lower())')
                    else:
                        value_checks.append(f'event.get("{panther_field}") == "{v}"')
                conditions.append(f'({" or ".join(value_checks)})')
            else:
                if modifier == 'contains':
                    conditions.append(f'"{value}".lower() in event.get("{panther_field}", "").lower()')
                elif modifier == 'endswith':
                    conditions.append(f'event.get("{panther_field}", "").lower().endswith("{value}".lower())')
                elif modifier == 'startswith':
                    conditions.append(f'event.get("{panther_field}", "").lower().startswith("{value}".lower())')
                else:
...
class AQLToSigma(ToSigmaConverter):
    ...
    def convert(self, source: str) -> SigmaRule:
        ...
        last_match = re.search(r'LAST\s+(\d+)\s+(MINUTE|HOUR|DAY|WEEK|MONTH)S?', source, re.IGNORECASE)
        if last_match:
        ...
        # Parse GROUP BY
        group_by_match = re.search(r'GROUP\s+BY\s+([^HAVING\nORDER\nLIMIT]+)', source, re.IGNORECASE)
        if group_by_match:
            sigma.group_by = [FieldMapper.to_canonical(f.strip(), "aql") for f in group_by_match.group(1).split(',')]
        ...
        if select_match:
            ...
            agg_pattern = r'(COUNT|SUM|AVG|MIN|MAX|UNIQUECOUNT)\s*\(\s*(\*|\w+)\s*\)(?:\s+AS\s+(\w+))?'
            agg_matches = re.findall(agg_pattern, select_clause, re.IGNORECASE)
            for func, agg_field, alias in agg_matches:
                sigma.aggregations.append(Aggregation(
                    function=func.upper(),
            ...
            if select_clause.strip() != '*':
                ...
                for part in select_clause.split(','):
                    ...
                    if not re.match(r'(COUNT|SUM|AVG|MIN|MAX|UNIQUECOUNT)\s*\(', part, re.IGNORECASE):
                        # Get field name (last word, handles "field AS alias")
                        field_name = part.split()[-1] if ' AS ' not in part.upper() else part.split()[0]
        ...
        # Parse WHERE clause
        where_match = re.search(r'WHERE\s+(.+?)(?:GROUP BY|ORDER BY|HAVING|LIMIT|START|STOP|LAST|$)', source, re.IGNORECASE | re.DOTALL)
        if where_match:
            where_clause = where_match.group(1)
...
class SigmaToAQL(FromSigmaConverter):
    ...
    def convert(self, sigma: SigmaRule) -> str:
        ...
        # Build SELECT clause
        fields = sigma.fields or ["sourceip", "destinationip", "username", "LOGSOURCENAME(logsourceid)", "starttime"]
...
class SQLToSigma(ToSigmaConverter):
    ...
    def convert(self, source: str) -> SigmaRule:
        ...
        # Parse WHERE clause
        where_match = re.search(r'WHERE\s+(.+?)(?:ORDER BY|GROUP BY|LIMIT|;|$)', source, re.IGNORECASE | re.DOTALL)
        if where_match:
            where_clause = where_match.group(1)
...
class CQLToSigma(ToSigmaConverter):
    ...
    def convert(self, source: str) -> SigmaRule:
        ...
        for filter_clause in filter_matches:
            ...
            for field, value in eq_matches:
                canonical = self._normalize_field(field)
            ...
            for field, value in neq_matches:
                canonical = self._normalize_field(field)
