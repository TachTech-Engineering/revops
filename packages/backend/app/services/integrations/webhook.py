...
class WebhookConnector(ActionConnector):
    ...
    async def execute(self, config: dict, alert_data: dict) -> ActionResult:
        url = config.get("url")
        if not url:
            return ActionResult(success=False, message="Missing webhook URL", error="url is required")
    ...
    def _build_payload(self, webhook_type: str, config: dict, alert_data: dict) -> dict:
        ...
        if webhook_type == "slack":
            ...
            color = {"CRITICAL": "FF0000", "HIGH": "FFA500", "MEDIUM": "FFFF00"}.get(severity, "808080")
            return {
                "@type": "MessageCard",
