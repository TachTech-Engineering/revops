...
class UniFiSyslogConnector(DataSourceConnector):
    ...
    MITRE_MAPPINGS = {
        ...
        "config_change": {"tactics": ["Defense Evasion", "Persistence"], "techniques": ["T1562", "T1098"]},
    ...
    def _normalize_api_alarm(self, alarm: dict[str, Any]) -> Optional[NormalizedAlert]:
    ...
    def _build_api_event_title(self, event_key: str, event: dict) -> str:
        ...
        # Fallback titles
        titles = {
            "EVT_WU_Connected": f"Client connected: {event.get('user', event.get('guest', 'unknown'))}",
            "EVT_WU_Disconnected": f"Client disconnected: {event.get('user', event.get('guest', 'unknown'))}",
    ...
    def _build_title(self, event_type: str, data: dict) -> str:
        ...
        titles = {
            "firewall_block": lambda d: f"Firewall Block: {d.get('src_ip', '?')} -> {d.get('dst_ip', '?')}:{d.get('dst_port', '?')}",
            "ids_alert": lambda d: f"IDS Alert: {d.get('signature', 'Unknown threat')}",
            "wireless_assoc": lambda d: f"Wireless {d.get('action', 'event').title()}: {d.get('client_mac', '?')}",
            "wireless_roam": lambda d: f"Client Roam: {d.get('client_mac', '?')} to {d.get('to_ap', '?')}",
            "admin_login": lambda d: f"Admin {d.get('action', 'event').title()} from {d.get('src_ip', 'unknown')}",
            "dhcp_event": lambda d: f"DHCP: {d.get('action', '?')} {d.get('ip', '')} {d.get('mac', '')}",
            "vpn_event": lambda d: f"VPN {d.get('action', 'event').title()}: {d.get('peer', 'unknown')}",
            "config_change": lambda d: f"Config {d.get('action', 'changed')} by {d.get('user', 'unknown')}",
            "switch_port": lambda d: f"Port {d.get('port', '?')}: {d.get('action', '?')}",
            "dpi_event": lambda d: f"DPI: {d.get('app_name', 'unknown')} detected for {d.get('client_mac', '?')}",
            "threat_detection": lambda d: f"Threat Detected: {d.get('threat_type', 'unknown')} from {d.get('src_ip', '?')}",
            "honeypot": lambda d: f"Honeypot Hit: {d.get('src_ip', '?')} on port {d.get('port', '?')}",
    ...
    def _build_description(self, event_type: str, data: dict) -> str:
        ...
        base += f"Facility: {syslog.get('facility', '?')}, Severity: {syslog.get('severity', '?')}\n\n"
