...
class PantherCodeGenerator:
    ...
    def __init__(self, analysis: AnalysisResult, transform: TransformResult) -> None:
        """
    ...
    def generate(self, rule_id: str, class_name: str | None = None) -> GeneratedRule:
        """
    ...
    def _generate_source_code(self, rule_id: str, class_name: str, log_types: list[str]) -> str:
...
'''
    ...
    def _generate_imports(self) -> str:
        ...
        if helper_imports:
            imports.append(f"from panther_sdk.detections import {', '.join(helper_imports)}")
    ...
    def _generate_class(self, rule_id: str, class_name: str, log_types: list[str]) -> str:
    ...
    def _generate_rule_method(self) -> str:
        ...
        if condition == "True":
            ...
            # FAIL CLOSED: nothing was converted into a per-event condition.
            # Returning True would fire on every event of the log type; an
            # inert rule with a loud TODO is the safe failure mode.
            parts.append("        # TODO: No conditions could be converted from the SPL.")
            parts.append("        # This rule is INERT (never fires) until you add the")
            parts.append("        # detection logic from the original query above.")
    ...
    def _generate_dedup_method(self) -> str:
        ...
        if self.transform.dedup_expression:
            ...
            if len(self.analysis.dedup_fields) == 1:
                ...
                field_parts = [self._get_field_access_simple(f) for f in self.analysis.dedup_fields]
                parts.append(f"        parts = [{', '.join(field_parts)}]")
    ...
    def _generate_alert_context_method(self) -> str:
        ...
        for field_name in self.transform.alert_context_fields:
            # Clean field name for dict key
            key = field_name.replace(".", "_")
            field_access = self._get_field_access_simple(field_name)
    ...
    def _generate_test_code(self, rule_id: str, class_name: str, log_types: list[str]) -> str:
